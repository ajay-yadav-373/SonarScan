...
echo "cvfti_startip:FTI startup ..."
su - $FTI_USER -c "LD_LIBRARY_PATH=$FASTSEARCH:lib; export LD_LIBRARY_PATH  FASTSEARCH;$FASTSEARCH/bin/setupenv.sh; $FASTSEARCH/bin/nctrl  start indexer"

ret=$?
if [ "$ret" -ne 0 ]; then
	echo "Full Text Index startup has failed with error code $ret"
else
	echo "Full Text Index startup is completed"
fi
exit $ret
